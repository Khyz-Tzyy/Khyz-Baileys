...
function normalizeJid(input) {
    ...
    if (Array.isArray(input)) return normalizeJid(input[0]);
    if (typeof input === 'object' && input.jid) return normalizeJid(input.jid);
    if (typeof input === 'object' && input.user) {
        return input.server ? `${input.user}@${input.server}` : `${input.user}@s.whatsapp.net`;
    }
    if (typeof input === 'number') return `${input}@s.whatsapp.net`;
    return null;
}
...
function normalizeContextInfo(contextInfo) {
    if (!contextInfo) return contextInfo;
    if (contextInfo.participant) {
        contextInfo.participant = normalizeJid(contextInfo.participant);
    }
...
exports.normalizeContextInfo = normalizeContextInfo;
