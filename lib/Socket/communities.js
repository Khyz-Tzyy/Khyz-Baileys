...
// Ported from the official WhiskeySockets/Baileys `communities.ts` socket layer.
// Adapted 1:1 into this project's compiled (CJS) module style so it slots into
// the existing Socket require()-chain without needing a TypeScript source tree.
const makeCommunitiesSocket = (config) => {
    ...
    const communityFetchAllParticipating = async () => {
        ...
        const data = {};
        const communitiesChild = (0, generic_utils_1.getBinaryNodeChild)(result, 'communities');
        if (communitiesChild) {
            const communities = (0, generic_utils_1.getBinaryNodeChildren)(communitiesChild, 'community');
    ...
    async function parseGroupResult(node) {
        const groupNode = (0, generic_utils_1.getBinaryNodeChild)(node, 'group');
        if (groupNode) {
            try {
                ...
                logger_1.default.error({ error }, 'error parsing group metadata from community result');
    ...
    sock.ws.on('CB:ib,,dirty', async (node) => {
        const dirtyChild = (0, generic_utils_1.getBinaryNodeChild)(node, 'dirty');
        if (!dirtyChild || dirtyChild.attrs.type !== 'communities') {
            return;
        }
    ...
    return {
        ...
        communityCreate: async (subject, body) => {
            const descriptionId = (0, generics_1.generateMessageID)().substring(0, 12);
        ...
        communityCreateGroup: async (subject, participants, parentCommunityJid) => {
            const key = (0, generics_1.generateMessageIDV2)();
            const result = await communityQuery('@g.us', 'set', [
                {
                    tag: 'create',
                    attrs: { subject, key },
        ...
        communityFetchLinkedGroups: async (jid) => {
            ...
            const subGroupsNode = (0, generic_utils_1.getBinaryNodeChild)(result, 'sub_groups');
            if (subGroupsNode) {
                const groupNodes = (0, generic_utils_1.getBinaryNodeChildren)(subGroupsNode, 'group');
        ...
        communityRequestParticipantsList: async (jid) => {
            ...
            const node = (0, generic_utils_1.getBinaryNodeChild)(result, 'membership_approval_requests');
            const participants = (0, generic_utils_1.getBinaryNodeChildren)(node, 'membership_approval_request');
        ...
        communityRequestParticipantsUpdate: async (jid, participants, action) => {
            ...
            const node = (0, generic_utils_1.getBinaryNodeChild)(result, 'membership_requests_action');
            const nodeAction = (0, generic_utils_1.getBinaryNodeChild)(node, action);
            const participantsAffected = (0, generic_utils_1.getBinaryNodeChildren)(nodeAction, 'participant');
        ...
        communityParticipantsUpdate: async (jid, participants, action) => {
            ...
            const node = (0, generic_utils_1.getBinaryNodeChild)(result, action);
            const participantsAffected = (0, generic_utils_1.getBinaryNodeChildren)(node, 'participant');
            ...
            await communityQuery(jid, 'set', [
                {
                    tag: 'description',
                    attrs: {
                        ...(description ? { id: (0, generics_1.generateMessageID)() } : { delete: 'true' }),
                        ...(prev ? { prev } : {})
                    },
        ...
        communityInviteCode: async (jid) => {
            ...
            const inviteNode = (0, generic_utils_1.getBinaryNodeChild)(result, 'invite');
        ...
        communityAcceptInvite: async (code) => {
            ...
            const result = (0, generic_utils_1.getBinaryNodeChild)(results, 'community');
        ...
        communityAcceptInviteV4: ev.createBufferedFunction(async (key, inviteMessage) => {
            ...
            if (key.id) {
                inviteMessage = { ...inviteMessage, inviteExpiration: 0, inviteCode: '' };
            ...
            await upsertMessage({
                key: {
                    ...
                    id: (0, generics_1.generateMessageIDV2)(sock.user?.id),
                    fromMe: false,
                ...
                participant: key.remoteJid,
                messageTimestamp: (0, generics_1.unixTimestampSeconds)()
            }, 'notify');
...
const extractCommunityMetadata = (result) => {
    const community = (0, generic_utils_1.getBinaryNodeChild)(result, 'community');
    const descChild = (0, generic_utils_1.getBinaryNodeChild)(community, 'description');
    let desc;
    let descId;
    if (descChild) {
        desc = (0, generic_utils_1.getBinaryNodeChildString)(descChild, 'body');
        descId = descChild.attrs.id;
    }
    const communityId = community.attrs.id?.includes('@')
        ? community.attrs.id
        : (0, WABinary_1.jidEncode)(community.attrs.id || '', 'g.us');
    const eph = (0, generic_utils_1.getBinaryNodeChild)(community, 'ephemeral')?.attrs.expiration;
    const memberAddMode = (0, generic_utils_1.getBinaryNodeChildString)(community, 'member_add_mode') === 'all_member_add';
    const metadata = {
        id: communityId,
        subject: community.attrs.subject || '',
        subjectOwner: community.attrs.s_o,
        subjectTime: Number(community.attrs.s_t || 0),
        size: (0, generic_utils_1.getBinaryNodeChildren)(community, 'participant').length,
        creation: Number(community.attrs.creation || 0),
        owner: community.attrs.creator ? (0, WABinary_1.jidNormalizedUser)(community.attrs.creator) : undefined,
        desc,
        descId,
        linkedParent: (0, generic_utils_1.getBinaryNodeChild)(community, 'linked_parent')?.attrs.jid || undefined,
        restrict: !!(0, generic_utils_1.getBinaryNodeChild)(community, 'locked'),
        announce: !!(0, generic_utils_1.getBinaryNodeChild)(community, 'announcement'),
        isCommunity: !!(0, generic_utils_1.getBinaryNodeChild)(community, 'parent'),
        isCommunityAnnounce: !!(0, generic_utils_1.getBinaryNodeChild)(community, 'default_sub_community'),
        joinApprovalMode: !!(0, generic_utils_1.getBinaryNodeChild)(community, 'membership_approval_mode'),
        memberAddMode,
        participants: (0, generic_utils_1.getBinaryNodeChildren)(community, 'participant').map(({ attrs }) => ({
            id: attrs.jid,
            admin: attrs.type || null
        })),
        ephemeralDuration: eph ? +eph : undefined,
        addressingMode: (0, generic_utils_1.getBinaryNodeChildString)(community, 'addressing_mode')
    };
    return metadata;
};
