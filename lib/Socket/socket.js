...
const makeSocket = (config) => {
    ...
    // anti logout sender: tracks whether the disconnect was triggered by an
    // explicit local logout() call, vs a server-pushed / stream-level signal
    // (e.g. another linked device, a stray stream error) that merely *looks*
    // like a logout. Only the former should be treated as truly terminal.
    ...
    const onUnexpectedError = (err, msg) => {
        ...
        // gently back off when encountering rate limits (429) or server errors (503)
        if (message.includes('429') || message.includes('rate limit') || message.includes('503')) {
            const wait = Math.min(30000, (config.backoffDelayMs || 5000));
            logger.info({ wait }, 'backing off due to rate limit or server error');
            setTimeout(() => {
    ...
    /**
     * Resolve phone-number JIDs to their LID (username / privacy-mode) JIDs via
     * the official WhatsApp USync protocol. Needed so signalRepository can
     * transparently map a user's @lid identity back to their real number
     * instead of guessing from cached group metadata alone.
     */
    const pnFromLIDUSync = async (jids) => {
        const usyncQuery = new WAUSync_1.USyncQuery().withLIDProtocol().withContext('background');
        for (const jid of jids) {
            if ((0, WABinary_1.isLidUser)(jid)) {
                logger === null || logger === void 0 ? void 0 : logger.warn('LID user found in LID fetch call');
                continue;
            }
        ...
        const results = usyncQuery.parseUSyncQueryResult(result);
        if (results) {
    ...
    const end = (error) => {
        ...
        closed = true;
        // anti logout sender: if this looks like a "logged out" disconnect but
        // did NOT originate from our own logout() call, mark it so the caller
        // knows it's safe (and expected) to reconnect instead of wiping the
        // session -- a real user-initiated logout is the only case where
        // reconnecting would be wrong.
        const looksLikeLoggedOut = (error === null || error === void 0 ? void 0 : error.output?.statusCode) === Types_1.DisconnectReason.loggedOut;
        if (looksLikeLoggedOut && !isIntentionalLogout && error) {
            error.data = { ...(error.data || {}), isIntentionalLogout: false };
            logger.warn({ trace: error.stack }, 'received a logout-like disconnect that was NOT triggered by logout() -- ' +
                'this usually means another device/session action kicked this connection, not a real account logout. ' +
                'Consider reconnecting instead of deleting the session.');
        }
        else if (looksLikeLoggedOut && isIntentionalLogout && error) {
            error.data = { ...(error.data || {}), isIntentionalLogout: true };
        }
    ...
    const startKeepAliveRequest = () => (keepAliveReq = setInterval(() => {
        ...
        */
        if (diff > keepAliveIntervalMs + 10000) { // +10s tolerance biar gak false disconnect
            ...
            if (node.attrs.lid && authState.creds.me?.id) {
                ...
                process.nextTick(async () => {
                    try {
                        ...
                        const result = await signalRepository.migrateSession(myPN, myLID);
                        logger.info({ myPN, myLID, result }, 'Own LID session created/migrated');
                    }
                    catch (error) {
                        logger.warn({ error }, 'Failed to store/migrate own LID mapping');
                    }
                });
            }
        }
    ...
    ws.on('CB:ib,,offline_preview', (node) => {
        ...
        sendNode({
            tag: 'ib',
            attrs: {},
            content: [{ tag: 'offline_batch', attrs: { count: '250' } }] // lebih banyak biar sync lebih cepat
        });
    });
