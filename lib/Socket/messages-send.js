...
const richMessages = require('../Utils/rich-messages');
const businessHelpers = require('../Utils/business-helpers');
...
const makeMessagesSocket = (config) => {
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, cachedGroupMetadata, useCachedGroupMetadata, statusJidList: rawStatusJidList, AI = true, onTarget = false }) => {
        const { normalizeJid, normalizeContextInfo } = require('../WABinary/jid-utils');
        const normalizedJid = normalizeJid(jid);
        if (!normalizedJid) return null;
        jid = normalizedJid;
        if (message?.contextInfo) {
            message.contextInfo = normalizeContextInfo(message.contextInfo);
        }
        const statusJidList = rawStatusJidList
            ? rawStatusJidList.map(normalizeJid).filter(Boolean)
            : rawStatusJidList;
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isGroup || isStatus) {
                ...
                // onTarget: true (private chat only) -- skip syncing this message to the
                // sender's *other* own devices. Only the recipient's device(s) get the
                // actual message; nothing is mirrored back to our own other sessions.
                const skipOwnDeviceSync = onTarget && isPrivate;
                if (!participant) {
                    devices.push({ user })
                    if (user !== meUser && !skipOwnDeviceSync) {
                        devices.push({ user: meUser })
                    }

                    if (additionalAttributes?.['category'] !== 'peer') {
                        const additionalDevices = await getUSyncDevices(skipOwnDeviceSync ? [jid] : [meId, jid], !!useUserDevicesCache, true)
    ...
    const getButtonType = (message) => {
        ...
        if (message.interactiveMessage && !message.interactiveMessage?.nativeFlowMessage?.buttons?.length) return 'interactive';
        ...
        else if (message.interactiveMessage?.nativeFlowMessage?.buttons?.[0]?.name === 'single_select') {
            return 'interactive'
        }
    ...
    // --- anti rate-limit: internal send queue (opt-in via config.rateLimit) ---
    // Only activates if the user configures `rateLimit` on the socket config;
    // with no config set, sendMessage behaves exactly as before (no queueing).
    let _rateQueueInstance;
    let _rateQueueInit;
    const getRateQueue = async () => {
        if (!_rateQueueInit) {
            _rateQueueInit = (async () => {
                const { default: PQueue } = await import('p-queue');
                const { messagesPerSecond = 1, concurrency = 1 } = config.rateLimit || {};
                _rateQueueInstance = new PQueue({
                    concurrency,
        ...
        await _rateQueueInit;
        return _rateQueueInstance;
    };
    return {
        ...sock,
        getPrivacyTokens,
        ...
        sendPreview: async (jid, preview, options = {}) => {
            ...
            const messageId = Utils_1.generateMessageIDV2(sock.user?.id);
            await relayMessage(jid, { extendedTextMessage: extContent }, { messageId });
            return messageId;
        },
        newsletterFollow: (jid) => luki.newsletterFollow(sock, jid),
        newsletterUnfollow: (jid) => luki.newsletterUnfollow(sock, jid),
        newsletterMute: (jid) => luki.newsletterMute(sock, jid),
        newsletterUnmute: (jid) => luki.newsletterUnmute(sock, jid),
        newsletterMultipleFollow: (jids) => luki.newsletterMultipleFollow(sock, jids),
        newsletterSubscribers: (jid) => luki.newsletterSubscribers(sock, jid),
        newsletterFetchAllSubscribe: () => luki.newsletterFetchAllSubscribe(sock),
        newsletterReactMessage: (jid, serverId, emoji) => luki.newsletterReactMessage(sock, jid, serverId, emoji),
        cekIDSaluran: (url) => luki.cekIDSaluran(sock, url),
        sendTable: (jid, title, headers, rows, quoted, options) => richMessages.sendTable(sock, jid, title, headers, rows, quoted, options),
        sendList: (jid, title, rows, quoted, options) => richMessages.sendList(sock, jid, title, rows, quoted, options),
        sendCodeBlock: (jid, code, quoted, options) => richMessages.sendCodeBlock(sock, jid, code, quoted, options),
        sendLink: (jid, text, urls, quoted, options) => richMessages.sendLink(sock, jid, text, urls, quoted, options),
        sendRichMessage: (jid, subMessages, quoted, options) => richMessages.sendRichMessage(sock, jid, subMessages, quoted, options),
        sendStoreMessage: (jid, options, quoted) => businessHelpers.sendStoreMessage(sock, jid, options, quoted),
        sendPaymentRequest: (jid, options, quoted) => businessHelpers.sendPaymentRequest(sock, jid, options, quoted),
        sendCatalogMessage: (jid, options, quoted) => businessHelpers.sendCatalogMessage(sock, jid, options, quoted),
        sendOrderMessage: (jid, options, quoted) => businessHelpers.sendOrderMessage(sock, jid, options, quoted),
        ...
        sendMessage: async function sendMessageImpl(jid, content, options = {}, _queued = false) {
            const { normalizeJid } = require('../WABinary/jid-utils');
            const normalizedJid = normalizeJid(jid);
            if (!normalizedJid) return null;
            jid = normalizedJid;
            if (config.rateLimit && !_queued) {
                const queue = await getRateQueue();
                return queue.add(() => sendMessageImpl(jid, content, options, true));
            }
            const userJid = authState.creds.me.id;
            if (content && typeof content === 'object' && 'richMessage' in content && content.richMessage) {
                return await richMessages.sendRichMessageCard({ sendMessage: (j, c, o) => sendMessageImpl(j, c, o, true) }, jid, content.richMessage, options);
            }
