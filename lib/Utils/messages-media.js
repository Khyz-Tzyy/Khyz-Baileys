...
// Upload streaming langsung dari file di disk lewat modul http/https bawaan
// Node, TANPA nge-buffer seluruh isi file ke memory dulu (beda sama upload
// axios bawaan di bawah, yang ngumpulin semua chunk ke RAM lewat
// Buffer.concat sebelum mulai upload — bisa berat kalau kirim video/file
// gede). Optional dipake: getWAUploadToServer tetep jalan normal kalau ini
// gak dipanggil.
...
const getWAUploadToServer = ({ customUploadHosts, fetchAgent, logger, options }, refreshMediaConn) => {
    return async (stream, { mediaType, fileEncSha256B64, newsletter, timeoutMs }) => {
        ...
        // Kalau input-nya stream (bukan Buffer kecil yang udah di-load), tulis
        // dulu ke file temp terus upload streaming lewat uploadWithNodeHttp —
        // biar gak nge-buffer semua isi file ke RAM kayak Buffer.concat lama.
        // Kalau gagal (misal disk penuh / limitasi environment), fallback ke
        // cara lama (buffer + axios) biar upload tetep jalan.
        let reqBody;
        let tempFilePath;
        if (Buffer.isBuffer(stream)) {
            ...
            try {
                tempFilePath = (0, path_1.join)((0, os_1.tmpdir)(), `wa-upload-${(0, generics_1.generateMessageIDV2)()}`);
                ...
                logger.warn({ error }, 'streaming upload prep failed, falling back to in-memory buffer');
        ...
        for (const { hostname, maxContentLengthBytes } of hosts) {
            ...
            try {
                if (tempFilePath) {
                    const fileStats = await fs_1.promises.stat(tempFilePath);
                    if (maxContentLengthBytes && fileStats.size > maxContentLengthBytes) {
                        throw new boom_1.Boom(`Body too large for "${hostname}"`, { statusCode: 413 });
                    }
                    result = await uploadWithNodeHttp({
                        url,
                    ...
                    if (!reqBody) {
                        // fallback: baca ulang dari temp path yang gagal ditulis, atau
                        // stream udah kekonsumsi — re-collect ke buffer sebagai jalan terakhir.
                        const chunks = [];
                        for await (const chunk of stream) {
                            chunks.push(chunk);
                        }
        ...
        if (tempFilePath) {
            fs_1.promises.unlink(tempFilePath).catch(() => { });
        }
        if (!urls) {
