...
/**
 * Normalized result from any WA button response message.
 */
export declare type ButtonResponseResult = {
    /** Type of button: 'nativeFlow' | 'interactive' | 'buttons' | 'list' | 'template' */
    type: 'nativeFlow' | 'interactive' | 'buttons' | 'list' | 'template';
    /** Button ID — used for bot command matching */
    id: string;
    /** Display text shown to user */
    text: string;
    /** For template buttons: the index of the selected button */
    index?: number;
 ...
 */
export declare const getButtonResponse: (message: proto.IMessage | null | undefined) => ButtonResponseResult | null;

/**
 * Returns the button ID string, or '' if not a button response.
 * @example if (getButtonResponseId(msg.message) === 'menu_utama') { ... }
 */
export declare const getButtonResponseId: (message: proto.IMessage | null | undefined) => string;

/**
 * Returns the display text of the selected button, or '' if not a button response.
 */
export declare const getButtonResponseText: (message: proto.IMessage | null | undefined) => string;
