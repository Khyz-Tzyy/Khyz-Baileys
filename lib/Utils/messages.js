...
exports.assertMediaContent = exports.downloadMediaMessage = exports.aggregateMessageKeysNotFromMe = exports.getAggregateVotesInPollMessage = exports.updateMessageWithPollUpdate = exports.updateMessageWithReaction = exports.updateMessageWithReceipt = exports.getDevice = exports.extractMessageContent = exports.normalizeMessageContent = exports.getContentType = exports.generateWAMessage = exports.generateWAMessageFromContent = exports.generateWAMessageContent = exports.generateForwardMessageContent = exports.prepareDisappearingMessageSettingContent = exports.prepareWAMessageMedia = exports.generateLinkPreviewIfRequired = exports.extractUrlFromText = exports.getButtonResponseText = exports.getButtonResponseId = exports.getButtonResponse = void 0;
            ...
            annotations: [
                {
                    ...
                    newsletter: {
                        ...
                        serverMessageId: 0,
                        newsletterName: "Khyz - Information",
                        contentType: "UPDATE",
                    }
                }
            ]
...
const generateWAMessageContent = async (message, options) => {
    ...
    // interactiveButtons — key yang dipakai ourin-socket.js (dan banyak bot lain)
    // Build interactiveMessage proto dari array buttons dengan format nativeFlow WA
    if ('interactiveButtons' in message && !!message.interactiveButtons) {
        const buttons = message.interactiveButtons;
        const interactiveMsg = {};

        // Body text
        interactiveMsg.body = { text: message.caption || message.text || '' };

        // Footer
        if (message.footer) {
            interactiveMsg.footer = { text: message.footer };
        }

        // Header: image / video / document / text
        const firstMediaKey = Object.keys(m)[0];
        if (firstMediaKey && firstMediaKey !== 'conversation') {
            const headerType = firstMediaKey.replace('Message', '').toUpperCase();
            interactiveMsg.header = {
                hasMediaAttachment: true,
                [firstMediaKey]: m[firstMediaKey],
            };
            // Kalau ada imageMessage langsung assign semua field-nya
            if (firstMediaKey === 'imageMessage') {
                interactiveMsg.header.imageMessage = m.imageMessage;
            } else if (firstMediaKey === 'videoMessage') {
                interactiveMsg.header.videoMessage = m.videoMessage;
            } else if (firstMediaKey === 'documentMessage') {
                interactiveMsg.header.documentMessage = m.documentMessage;
            }
        } else if (message.header) {
            interactiveMsg.header = message.header;
        } else if (message.caption || message.text) {
            interactiveMsg.header = {
        ...
        // nativeFlowMessage — bungkus semua buttons sebagai nativeFlow
        interactiveMsg.nativeFlowMessage = {
            buttons: buttons.map(btn => {
                // Kalau sudah dalam format {name, buttonParamsJson} langsung pakai
                if (btn.name && btn.buttonParamsJson !== undefined) {
                    return { name: btn.name, buttonParamsJson: btn.buttonParamsJson };
                }
                // Kalau format lama {id, displayText} convert ke quick_reply
                if (btn.buttonId || btn.id) {
                    return {
                        name: 'quick_reply',
                        buttonParamsJson: JSON.stringify({
                            display_text: btn.displayText || btn.buttonText || btn.id || btn.buttonId,
                            id: btn.id || btn.buttonId,
                        }),
                    };
                }
                // Kalau ada name tapi belum ada buttonParamsJson
                return {
        ...
        m = { interactiveMessage: interactiveMsg };
    }
    ...
    // carousel: an array of swipeable interactive cards in a single message.
    // Each card: { title?, body?, footer?, image? (url), buttons?: [...] }
    // `buttons` per-card follow the same shorthand accepted by the regular
    // interactive/nativeFlow builder above ({ name, buttonParamsJson } or
    // the legacy { id/buttonId, displayText/buttonText }).
    if ('carousel' in message && Array.isArray(message.carousel) && message.carousel.length) {
        const toNativeFlowButtons = (buttons = []) => buttons.map(btn => {
            if (btn.name && btn.buttonParamsJson !== undefined) {
                return { name: btn.name, buttonParamsJson: btn.buttonParamsJson };
            }
            if (btn.buttonId || btn.id) {
        ...
        const cards = message.carousel.map(card => {
            const cardMsg = {
                body: card.body ? { text: card.body } : undefined,
                footer: card.footer ? { text: card.footer } : undefined,
                header: {
                    title: card.title || '',
                    hasMediaAttachment: !!card.image,
                    ...(card.image ? { imageMessage: { url: card.image } } : {})
                },
                nativeFlowMessage: {
                    buttons: toNativeFlowButtons(card.buttons),
                    messageParamsJson: ''
                }
            };
            return cardMsg;
...
const generateWAMessageFromContent = (jid, message, options) => {
    const { normalizeJid } = require('../WABinary/jid-utils');
...
// alias untuk kompatibilitas rich messages
const getContentTypeSafe = (content) => {
    if (!content) return undefined;
    if (content.botForwardedMessage) return 'botForwardedMessage';
    if (content.interactiveMessage) return 'interactiveMessage';
    return getContentType(content);
};
exports.getContentType = getContentType;
exports.getContentTypeSafe = getContentTypeSafe;
/**
...
const extractMessageContent = (content) => {
    ...
    content = (0, exports.normalizeMessageContent)(content);
    if (content === null || content === void 0 ? void 0 : content.interactiveMessage) {
        return { interactiveMessage: content.interactiveMessage };
    }
    if (content === null || content === void 0 ? void 0 : content.botForwardedMessage) {
        return { botForwardedMessage: content.botForwardedMessage };
    }
...
/** Update the message with a new event RSVP response */
const updateMessageWithEventResponse = (msg, update) => {
    ...
    const responses = (msg.eventResponses || [])
        .filter(r => (0, generics_1.getKeyAuthor)(r.eventResponseMessageKey) !== authorID);
    responses.push(update);
    msg.eventResponses = responses;
};
exports.updateMessageWithEventResponse = updateMessageWithEventResponse;
/**
 * Ngumpulin semua RSVP (GOING/NOT_GOING/MAYBE) di satu event message.
 * @param msg event creation message
 * @param meId JID kamu sendiri
 * @returns daftar tipe respons & siapa aja yang jawab
 */
...
/**
 * Extracts button response info from any WA button message type.
 * Handles: interactiveResponseMessage (nativeFlow), buttonsResponseMessage,
 * listResponseMessage, templateButtonReplyMessage.
 *
 * Returns null if the message is not a button response.
 *
 * @example
 * const resp = getButtonResponse(msg.message)
 * if (resp) console.log(resp.id, resp.text, resp.type)
 */
const getButtonResponse = (message) => {
    if (!message) return null;
    const m = (0, exports.normalizeMessageContent)(message);
    if (!m) return null;

    // 1. interactiveResponseMessage — format baru WA (nativeFlow / quick reply)
    if (m.interactiveResponseMessage) {
        ...
        // quick reply / plain interactive response tanpa nativeFlow
        return {
            type: 'interactive',
            id: body,
            text: body,
            raw: m.interactiveResponseMessage,
        };
    }

    // 2. buttonsResponseMessage — format lama (deprecated tapi masih diterima)
    if (m.buttonsResponseMessage) {
        const btn = m.buttonsResponseMessage;
        return {
            type: 'buttons',
            id: btn.selectedButtonId || '',
            text: btn.selectedDisplayText || btn.selectedButtonId || '',
            raw: btn,
        };
    }

    // 3. listResponseMessage — list / row selection
    ...
    // 4. templateButtonReplyMessage — template / hydrated button
    if (m.templateButtonReplyMessage) {
        const tmpl = m.templateButtonReplyMessage;
        return {
            type: 'template',
            id: tmpl.selectedId || '',
            text: tmpl.selectedDisplayText || tmpl.selectedId || '',
            index: tmpl.selectedIndex,
            raw: tmpl,
        };
    }
...
exports.getButtonResponse = getButtonResponse;

/**
 * Shorthand — returns only the button ID string, or '' if not a button response.
 * if (getButtonResponseId(msg.message) === 'menu_utama') { ... }
 */
const getButtonResponseId = (message) => {
    return (0, exports.getButtonResponse)(message)?.id || '';
};
exports.getButtonResponseId = getButtonResponseId;

/**
 * Shorthand — returns only the display text of the button, or '' if not a button response.
 */
const getButtonResponseText = (message) => {
    return (0, exports.getButtonResponse)(message)?.text || '';
};
exports.getButtonResponseText = getButtonResponseText;
